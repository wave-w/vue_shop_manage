...
<div>
        ...
        <el-table :data="rolesList" style="width: 100%" border stripe>
            <el-table-column type="expand">
                <template slot-scope="scope">
                    <el-row v-for="(item,index) in scope.row.children" :key="item.id" :class="{bottomborder:true,topborder:index==0,vcenter:true}">
                        <el-col :span="6">
                            <el-tag closable @close="todeltag(scope.row,item.id)">
                                {{item.authName}}
                            </el-tag><i class="el-icon-caret-right"></i>
                        </el-col>

                        <el-col :span="19">
                            <el-row v-for="(citem,index) in item.children" :key="citem.id" :class="{topborder:index!=0,vcenter:true}">
                                <el-col :span="6">
                                    <el-tag type="success" closable @close="todeltag(scope.row,citem.id)">
                                        {{citem.authName}}
                                    </el-tag>
                                    <i class="el-icon-caret-right"></i>
                                </el-col>
                                <el-col :span="18">
                                    <el-tag type="warning" v-for="(ccitem,index) in citem.children" :key="ccitem.id" :class="{topborder:index!=0}" closable @close="todeltag(scope.row,ccitem.id)">
                                        {{ccitem.authName}}
                                    </el-tag>
                                </el-col>
                            </el-row>
            ...
            <el-table-column prop="level" label="操作" width="300px">
                <template slot-scope="scope">
                    ...
                    <el-tooltip effect="dark" content="分配角色" placement="top" :enterable="false">
                        <el-button type="warning" icon="el-icon-setting" size="mini" @click="setright(scope.row)">分配角色</el-button>
                    </el-tooltip>
                </template>
            </el-table-column>
        </el-table>
    ...
    <el-dialog title="分配权限" :visible.sync="setrightVisible" width="30%" @close="setrightclose">
        <el-tree :data="rightsList" :props="defaultProps" show-checkbox node-key="id" default-expand-all :default-checked-keys="defaultkey" ref="treeref">
        </el-tree>
